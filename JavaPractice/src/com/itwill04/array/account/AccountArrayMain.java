...
public class AccountArrayMain {
	...
	public static void main(String[] args) {
		...
		Account[] accounts = { acc1, new Account(2222, "LEE", 58900, 0.5), new Account(3333, "PARK", 78000, 0.4),
				new Account(4444, "CHOI", 34000, 0.3), new Account(5555, "SIM", 66000, 0.7),
				new Account(6666, "HONG", 99000, 1.4), new Account(7777, "KIM", 87600, 0.6),
				new Account(8888, "SOO", 12300, 2.3), new Account(9999, "MIN", 41200, 4.1) };
		...
		/********************
		 * OPTION*********************** 1. accounts배열 객체에 새로생성한계좌객체 추가 A.
		 * accounts.length+1 개짜리 임시배열생성 B. accounts의 모든계좌객체 임시배열로 이동 C. 임시배열에 새로운
		 * Account객체추가 D. accounts에 임시배열주소대입
		 * 
		 * 2. accounts배열 객체에서 계좌번호 4444번계좌객체삭제 A. accounts 에서 8888계좌찾아서 null대입 B.
		 * accounts.length-1 개짜리 임시배열생성 C. accounts null이아닌 모든계좌 임시배열로이동 D. accounts 에
		 * 임시배열대입
		 */
		...
		Account[] redAccount = new Account[accounts.length - 1];
		for (int i = 0; i < accounts.length; i++) {
		...
		for (int j = 0; j < accounts.length-1; j++) {
			if (accounts[j] != null) {
				redAccount[j] = accounts[j];
			} else {redAccount[j] = accounts[j+1];}
		}
		accounts = redAccount;
