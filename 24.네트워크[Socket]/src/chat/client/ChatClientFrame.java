...
public class ChatClientFrame extends JFrame {
	...
	public ChatClientFrame() throws Exception {
		...
		Socket socket = new Socket("192.168.15.19", 8888);
	...
	/**************************************************************
	 * - 클라이언트쪽소켓을 사용하여 서버와 통신을 담당하는 클래스(VO) - 서버와연결된 소켓1개를 가지고있는클래스 - 클라이언트의
	 * 정보를가지고있는클래스 - 클라이언트당 1개의객체가생성
	 ****************************************************************/
	...
	public class Clientclient extends Thread {
		...
		/*
		 * 서버에서 보내는 데이터를 읽기
		 * 클라이언트 클라이언트는 하나여서 안에 스레드 작성
		 */
		@Override
		public void run() {
			try {
				while (true) {
					System.out.println("가.Clientclient: 서버로부터 오는 데이터를 읽기위해 스레드 무한대기(blocking)");
					String chatString = in.readLine();
					// BufferdReader로 읽기를 무한으로 대기
					System.out.println("가.Clientclient: 서버로부터 오는 데이터를 클라이언트 채팅창에 보여줌");
